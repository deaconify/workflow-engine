...
- **TaskCreate mandate (Step 0)**: Every workflow invocation calls `TaskCreate` with one task per workflow step (0, 1, 1b, 2, 2b, 3, 4, 4b, 4c, 4d, 5, 6, 7, 8). No step is "done" until its task is marked completed. Workflow is not complete until TaskList is empty AND the project's Step 8 merge policy (per `project-context.md`) has executed.
- **Interruption rule**: Mid-workflow questions get a brief answer then return to the current `in_progress` task. Never start a new issue's workflow with a non-empty TaskList from a prior issue — either finish the current workflow or explicitly abort it with user confirmation first.
- **No false terminations**: Reviewer APPROVE, close-out table approval, branch push, and PR-opened (for projects that use PRs) are all mid-flight milestones. Only Step 8 completion — merge policy executed, all `project-context.md` close-out obligations run, TaskList empty — ends the workflow. Projects without PRs commit per their merge policy and flow straight through Steps 5–8 with no pause point.
- **Reviewer claim verification (Step 4b)**: Reviewer is an agent and can hallucinate file paths, versions, and counts. The orchestrator MUST spot-check every reviewer finding about file existence, version strings, or counts before repeating it. File-existence claims verified via `Glob`/`Read`; count claims recounted from the IRD at Step 4d.
- **Constraint recount (Step 4d)**: The close-out IRD Compliance row is recounted from `brain/sessions/ird-{issue-number}.md` directly — never copy-pasted from the reviewer's table, because in-place fixes at Step 4 may have flipped statuses. Tiers reported separately (MUST / SHOULD / EXPLAIN) — never collapsed into a single ratio, never unlabeled.
...
- **IRD persistence (Persistence Gate)**: Enforced by the `ird-gate` PreToolUse hook. Step 2 creates sentinel `brain/sessions/.ird-pending-{issue}`; the hook blocks Edit/Write/MultiEdit on implementation files until it is removed. Step 2b removes the sentinel ONLY after (1) `brain/sessions/ird-{issue}.md` is written and re-read to verify, (2) the FULL IRD is posted as a GitHub comment and verified via `gh issue view --comments`, (3) the file is linted. Never remove the sentinel to unblock — fix the failing substep. See `standard-workflow.md` Step 2b. Deleted at Step 8.
